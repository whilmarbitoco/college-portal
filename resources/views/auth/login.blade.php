<x-app-layout>
    <div class="flex justify-center items-center px-4 min-h-screen sm:px-6 lg:px-8">
        <div class="p-6 w-full max-w-sm text-center rounded-lg shadow-xl bg-secondary-bg sm:p-8 sm:max-w-md">
            ...
            <form class="space-y-5 sm:space-y-6" method="POST" accept="{{ route('login') }}">
                @csrf
                ...
                <div class="flex flex-col gap-2 text-sm sm:flex-row sm:items-center sm:justify-between sm:gap-0">
                    ...
                    <div>
                        <a href="{{ route('password.request') }}"
                            class="font-medium transition-colors text-accent hover:text-accent-light">Forgot your
                            password?</a>
                    </div>
                </div>
